...
// คำ SQL สำหรับค้นหาข้อมูลในตาราง feedback โดยมีการกรองจากคำค้นหาใน ReportID, OrderID, ReportTopic และ ReportStatus
$sql = "SELECT ReportID, OrderID, ReportDate, ReportTopic, ReportExplanation, ReportImage, ReportStatus 
        FROM report
        WHERE 
            ReportID LIKE '%$search%' OR
            OrderID LIKE '%$search%' OR
            ReportTopic LIKE '%$search%' OR
            ReportStatus LIKE '%$search%' OR
            ReportExplanation LIKE '%$search%'
...
<body>
            ...
            <div class="customer-table-container">
                ...
                <div class="search-insert-row">
                    ...
                    <div class="button-group">
                        <a href="Review.php" class="admin-btn">
                            <ion-icon name="people-circle-outline"></ion-icon>
                            Review Management
                        </a>

                        <a href="RP_insert.php" class="insert-cus-btn">
                    ...
                    <tbody>
                        <?php if ($result->num_rows > 0): ?>
                            <?php while($row = $result->fetch_assoc()): ?>
                                <tr>
                                    <td><?= isset($row["ReportID"]) ? $row["ReportID"] : 'N/A' ?></td>
                                    <td><?= isset($row["OrderID"]) ? $row["OrderID"] : 'N/A' ?></td>
                                    <td><?= isset($row["ReportDate"]) ? $row["ReportDate"] : 'N/A' ?></td>
                                    <td><?= isset($row["ReportTopic"]) ? $row["ReportTopic"] : 'N/A' ?></td>
                                    <td><?= isset($row["ReportExplanation"]) ? $row["ReportExplanation"] : 'N/A' ?></td>
                                    <td><img src="<?= $row['ReportImage'] ?>" alt="Report Image" style="width: 150px; height: auto;"></td>
                                    <td><?= isset($row["ReportStatus"]) ? $row["ReportStatus"] : 'N/A' ?></td>
                                    <td class="actions">
                                        <div class="action-buttons">
                                            <a href="RP_edit.php?ReportID=<?= $row['ReportID'] ?>" class="edit-btn">Edit</a>
                                            <button type="button" class="delete-btn" onclick="openModal('<?= $row['ReportID'] ?>')">Delete</button>
                                        </div>
                                    </td>
                                </tr>
                            <?php endwhile; ?>
   ...
   <script>
    ...
    <div class="modal-content">
        ...
        <form method="post" action="RP_delete.php" id="deleteForm">
