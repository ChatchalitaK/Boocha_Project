...
// SQL ค้นหาข้อมูล roastlevel
$sql = "SELECT RoastLevelID, RoastLevelName
        FROM roastlevel
        WHERE 
            RoastLevelID LIKE '%$search%' OR
            RoastLevelName LIKE '%$search%'
...
<body>
   ...
   <div class="container">
    <div class="navigation">
     ...
     <div class="main">
            ...
            <!-- ตารางแสดงข้อมูล RoastLevel -->
            <div class="customer-table-container">
                <div class="top-row">
                   <h2>RoastLevel Management</h2>
                </div>

                <div class="search-insert-row">
                    <form method="get" action="roastlevel.php">
                        <div class="search">
                            <label>
                    ...
                    <a href="RL_insert.php" class="insert-cus-btn">
                        <ion-icon name="add-circle-outline"></ion-icon>
                        Create New RoastLevel
                    </a>
                    ...
                    <tbody>
                        <?php if ($result->num_rows > 0): ?>
                            <?php while($row = $result->fetch_assoc()): ?>
                                <tr>
                                    <td><?= $row["RoastLevelID"] ?></td>
                                    <td><?= $row["RoastLevelName"] ?></td>
                                    <td class="actions">
                                        <div class="action-buttons">
                                            <a href="RL_edit.php?RoastLevelID=<?= $row['RoastLevelID'] ?>" class="edit-btn">Edit</a>
                                            <button type="button" class="delete-btn" onclick="openModal('<?= $row['RoastLevelID'] ?>')">Delete</button>
                                        </div>
                                    </td>
                                </tr>
                            <?php endwhile; ?>
   ...
   <script>
    function openModal(roastLevelID) {
    document.getElementById('deleteRoastLevelID').value = roastLevelID;
    ...
    <div id="deleteModal" class="modal" style="display: none;">
        <div class="modal-content">
            <p>Are you sure you want to delete this roast level?</p>
            <form method="post" action="RL_delete.php" id="deleteForm">
                <input type="hidden" name="RoastLevelID" id="deleteRoastLevelID">
