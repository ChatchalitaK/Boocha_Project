...
$sql = "SELECT COUNT(*) AS total_sales FROM `order` WHERE orderdate >= CURDATE() - INTERVAL 7 DAY";
...
// ====== ดึงข้อมูล Best Selling Tea (7 วันล่าสุด)
$sql_best_tea = "
...
// ====== ดึงข้อมูล Least Selling Tea (7 วันล่าสุด)
$sql_least_tea = "
    SELECT tt.TeaTypeName
    FROM tea_type tt
    WHERE tt.TeaTypeID = (
...
if ($result_least_tea && $row_least_tea = $result_least_tea->fetch_assoc()) {
    $least_tea = $row_least_tea['TeaTypeName'] ?? "ไม่มีข้อมูล";
} else {
    $least_tea = "ไม่มีข้อมูล";
}

// ====== Best Sales Province (7 วันล่าสุด)
$sql_best_province = "
...
// ====== Least Sales Province (7 วันล่าสุด)
$sql_least_province = "
    SELECT pr.ProvinceName
    FROM province pr
    WHERE pr.ProvinceID = (
...
if ($result_least_province && $row_least_province = $result_least_province->fetch_assoc()) {
   ...
   <!-- Chart.js -->
    ...
    new Chart(lineCtx2, {
        ...
        options: {
            responsive: true,
            plugins: {
                title: { display: true, text: 'Earnings in Last 7 Days' }
            }
        }
    });
