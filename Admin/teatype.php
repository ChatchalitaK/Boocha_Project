...
// SQL ค้นหาข้อมูล tea_type
$sql = "SELECT TeaTypeID, TeaTypeName
        FROM tea_type
        WHERE 
            TeaTypeID LIKE '%$search%' OR
            TeaTypeName LIKE '%$search%'
...
<body>
    ...
    <div class="navigation">
     ...
     <div class="main">
            ...
            <div class="customer-table-container">
                <div class="top-row">
                    <h2>Tea Type Management</h2>
                </div>

                <div class="search-insert-row">
                    <form method="get" action="teatype.php">
                        <div class="search">
                            <label>
                    ...
                    <a href="TT_insert.php" class="insert-cus-btn">
                    ...
                    <tbody>
                        <?php if ($result->num_rows > 0): ?>
                            <?php while($row = $result->fetch_assoc()): ?>
                                <tr>
                                    <td><?= $row["TeaTypeID"] ?></td>
                                    <td><?= $row["TeaTypeName"] ?></td>
                                    <td class="actions">
                                        <div class="action-buttons">
                                            <a href="TT_edit.php?TeaTypeID=<?= $row['TeaTypeID'] ?>" class="edit-btn">Edit</a>
                                            <button type="button" class="delete-btn" onclick="openModal('<?= $row['TeaTypeID'] ?>')">Delete</button>
                                        </div>
                                    </td>
                                </tr>
                            <?php endwhile; ?>
   ...
   <script>
    function openModal(teaTypeID) {
        console.log('Opening modal for Tea Type ID:', teaTypeID);
        document.getElementById('deleteTeaTypeID').value = teaTypeID;
    ...
    <div id="deleteModal" class="modal" style="display: none;">
        <div class="modal-content">
            <p>Are you sure you want to delete this tea type?</p>
            <form method="post" action="TT_delete.php" id="deleteForm">
                <input type="hidden" name="TeaTypeID" id="deleteTeaTypeID">
